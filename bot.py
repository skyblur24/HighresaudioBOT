...
def normalizarUrl(url):
    url = re.sub(
        r"^https://(?:[\w-]+\.)?highresaudio\.com",
        "https://www.highresaudio.com",
        url.strip()
    )
    url = re.sub(
        r"(highresaudio\.com/)[a-z]{2}(/album/view/)",
        r"\1en\2",
        url
    )
    return url
...
def extract_info_block(page_text):
    idx_start = page_text.find("Album info")
    if idx_start == -1:
        return page_text
    idx_end = page_text.find("Album including", idx_start)
    if idx_end == -1:
        return page_text[idx_start:idx_start + 1500]
    idx_end += len("Album including") + 200
    return page_text[idx_start:idx_end]

def build_caption(artist, title, page_text, rate_string, hasBooklet, url):
    info_block = extract_info_block(page_text)
    hra_release = extract_field(info_block, "HRA-Release")
    label = extract_field(info_block, "Label")
    genre = extract_field(info_block, "Genre")
    subgenre = extract_field(info_block, "Subgenre")
    artist_field = extract_field(info_block, "Artist") or artist
    ...
    partes.append("")
    partes.append("Store: HIGHRESAUDIO")
    partes.append("")
    partes.append("Download Link:")
...
def main(config, url, status_msg_id=None):
    ...
    try:
        ...
        caption = build_caption(artist, title, page_text, rate_string, hasBooklet, url)

        if not extract_field(extract_info_block(page_text), "HRA-Release") and not extract_field(extract_info_block(page_text), "Label"):
